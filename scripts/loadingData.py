...
#        lastS, outFileName =dld.dowloadOutAnalysis(c2id[city])
        path = "../data%s/" % city
        outFileName = "out_analysis_%d_cr.txt" %c2id[city]
...
#metrics = ["TravelWithPenlaty", "AvgSOC", "AvgStationOccupancy"]
#metrics = ["ReroutePerc"]


#cdfList_bdst, cdfList_bdur, cdfList_pdu git, dict_df, log_df,\
#plt_home, path, mytt =  downloadAllStuff(c2id)


#x,y = metricVaryingZonesAndAcs(dict_df["Torino"],72,
#                               25, [0,25,50,75], "Deaths", ax="")
for city in c2id.keys():
    mytt=25
...
#                  save=False, onlyFF=True, path="../plot"+city+"/")
#    
...
#aggregateUtilizastionPerHour(['Vancouver', "Berlino", "Milano", "Torino"], save=True, 
#                                   path='../plotAggregated/')
